 ...
 */
export async function unlockUserShapes(_userId: string): Promise<void> {
  ...
  // Unlocking all shapes for user: ${userId}
}
 ...
 */
export async function getShapeLocks(_shapeIds: string[]): Promise<Record<string, ShapeLock>> {
 ...
 */
export async function cleanupStaleLocks(_staleThresholdMs: number = 5 * 60 * 1000): Promise<string[]> {
  ...
  // Cleaning up locks older than ${staleThresholdMs}ms
  return []
}
